...
const getFollowingPeopleUserIdsOfUser = async (username) => {
...
app.get("/user/tweets/feed/", authentication, async (request, response) => {
  ...
  const followingPeopleIds = await getFollowingPeopleUserIdsOfUser(username);
...
app.get("/user/followers/", authentication, async (request, response) => {
  ...
  const followers = await db.all(getFollowerUserQuery);
...
app.get(
  ...
  async (request, response) => {
    ...
    WHERE tweet.tweet_id = ${tweetId} AND tweet.user_id =${userId};`;
    const tweet = await db.all(getTweetQuery);
    response.send(tweet);
  }
);
...
app.get(
  ...
  async (request, response) => {
    ...
    const likedUserArray = likedUsers.map((eachUser) => eachUser.username);
    response.send({ likes: likedUserArray });
  }
);
...
app.get(
  ...
  async (request, response) => {
    ...
    WHERE tweet_id = ${tweetId};`;
    const repliedUsers = await db.all(replyQuery);
